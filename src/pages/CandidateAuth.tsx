...
import { CircleUser as UserCircle, Mail, Lock, ArrowLeft } from 'lucide-react';

interface CandidateAuthProps {
  onBack: () => void;
}

export default function CandidateAuth({ onBack }: CandidateAuthProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        await signUp(email, password, 'candidate');
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <h2 className="text-3xl font-bold text-slate-800 text-center mb-2">
            {isLogin ? 'Welcome Back' : 'Join as Candidate'}
          </h2>
